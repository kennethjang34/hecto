...
impl Row {
    pub fn render(&self, start: usize, end: usize) -> String {
            ...
            .take(end - start)
        {
            if let Some(c) = grapheme.chars().next() {
                ...
                if index == 0 || highlighting_type != current_highlighting {
    ...
    pub fn find(&self, query: &str, at: usize, direction: SearchDirection) -> Option<usize> {
        ...
        //end index not included for search
    ...
    fn highlight_multiline_comment(
        ...
        if opts.multiline_comments() && c == '/' && *index < chars.len() - 1 {
            if let Some(next_char) = chars.get(index.saturating_add(1)) {
                if *next_char == '*' {
                    //TODO
                    //make the following line more robust
                    //self.string[*index + 2..].find("*/") might not work with non-ascii character
                    //(character with more than one byte)
                    //because find(&str) returns byte index of the str, not grapheme index
                    let closing_index =
    ...
    pub fn highlight(
        ...
        let mut index = 0;
        let mut prev_ml_comment = start_with_comment;
        ...
        while let Some(c) = chars.get(index) {
            if self.highlight_multiline_comment(&mut index, &opts, *c, &chars) {
                prev_ml_comment = true;
                continue;
            }
            prev_ml_comment = false;
        ...
        if prev_ml_comment && &self.string[self.string.len().saturating_sub(2)..] != "*/" {
            return true;
        }
        false
    }
